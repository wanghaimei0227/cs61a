    ...
    def vend(self):
        if self.stock:
            ...
            if difference > 0:
                ...
                if difference != 0:
                    message += ' and ${} change'.format(-difference)
                self.balance = 0
                self.stock -= 1
                return message + '.'
        else:
...
def is_bst(t):
    ...
    """
    def bst_min(t):
        """Return the min of t, if t has the structure of a valid BST."""
        if t.is_leaf():
            return t.label
        return min(t.label, bst_min(t.branches[0]))

    def bst_max(t):
        """Return the max of t, if t has the structure of a valid BST."""
        if t.is_leaf():
            return t.label
