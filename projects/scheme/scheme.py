...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        name = target.first
        expr = Pair(target.rest, expressions.rest)
        env.define(name, do_lambda_form(expr, env))
        return name
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    if not expressions:
        return True
    expr_temp = expressions
    while expr_temp:
        last_value = scheme_eval(expr_temp.first, env)
        if is_false_primitive(last_value):
            return False
        expr_temp = expr_temp.rest
    return last_value
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    expr_temp = expressions
    while expr_temp:
        value = scheme_eval(expr_temp.first, env)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    while bindings:
        name_and_val = bindings.first
        validate_form(name_and_val, 2, 2)
        name = name_and_val.first
        value = eval_all(name_and_val.rest, env)
