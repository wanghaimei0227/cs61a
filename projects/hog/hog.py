...
def roll_dice(num_rolls, dice=six_sided):
    ...
    roll_one = False
    while k <= num_rolls:
        curr_dice_outcome = dice()
        if curr_dice_outcome == 1:
            roll_one = True
        score += curr_dice_outcome
        k += 1
    if roll_one:
        return 1
    else:
...
def swine_align(player_score, opponent_score):
    ...
    potential_gcd = min_score
    if player_score == 0 or opponent_score == 0:
        return False
    while potential_gcd >= 10:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        new_last_score = last_score
        new_running_high = running_high
        if who == 0:
            # player0
            interest = score0 - new_last_score
            new_last_score = score0
        else:
            interest = score1 - new_last_score
            new_last_score = score1
        if interest > new_running_high:
            new_running_high = interest
            print(interest, "point(s)! The most yet for Player", who)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=100):
    ...
    k = 1
    max_average_num = 0
    make_average_num = make_averaged(roll_dice, trials_count)
    while k <= 10:
        curr_average_num = make_average_num(k, dice)

        if curr_average_num > max_average_num:
            max_average_num = curr_average_num
            highest_num_rolls = k
...
def run_experiments():
    ...
    # if True:  # Change to True to test always_roll(1)
    ...
    # if True:  # Change to True to test always_roll(9)
    #     print('always_roll(9) win rate:', average_win_rate(always_roll(9)))
    # if True:  # Change to True to test always_roll(10)
    ...
    if False:  # Change to True to test final_strategy1
        print('final_strategy1 win rate:', average_win_rate(final_strategy1))
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    cutoff = 9
    num_rolls = 6
    if_free_bacon = free_bacon(score)
    if if_free_bacon + score >= 100:
        return 0
    if score > 96:
        cutoff = 2
        num_rolls = 1
...
# def final_strategy1(score, opponent_score):
