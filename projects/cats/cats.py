...
def about(topic):
    ...
    def select(paragraph):
        para = split(lower(remove_punctuation(paragraph)))
        for t in topic:
            for p in para:
                if t == p:
                    return True
...
def report_progress(typed, prompt, user_id, send):
    ...
    # send our reprot with the 'send' function
    send(report)
    return report.get('progress')
...
def time_per_word(times_per_player, words):
    ...
    for p in range(len(times_per_player)):
        ...
        for i in range(len(times_per_player[p]) - 1):
            p_times += [times_per_player[p][i+1] - times_per_player[p][i]]
        times += [p_times]
...
def fastest_words(game):
    ...
    for i in word_indices:
        ...
        fastest_words[fastest_player] += [word_at(game, i)]
