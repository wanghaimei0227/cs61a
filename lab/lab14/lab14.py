...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        if not check.deposited and check.payable_to == self.holder:
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            string, cost = helper_align(skeleton_idx+1, code_idx+1)
            possibilities.append((skel_char+string, cost))

        # Insert
        edit = "".join("+[" + code_char + "]")
        string, cost = helper_align(skeleton_idx, code_idx+1)
        possibilities.append((edit+string, cost+1))

        # Delete
        edit = "".join("-[" + skel_char + "]")
        string, cost = helper_align(skeleton_idx+1, code_idx)
        possibilities.append((edit+string, cost+1))
        print("DEBUG:", possibilities)
...
def reverse(lst):
    ...
    # return foldl(lst,lambda a, b: Link(b, a), Link.empty)
